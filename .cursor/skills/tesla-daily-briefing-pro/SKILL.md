...
- 仅提供日期且无 JSON 时：自动尝试从 GitHub Pages 拉取：
  - **全量语义筛选结果（优先，体量小、已是匹配项）**：`https://arthurwang2.github.io/TrendRadar2/report/meta/semantic_filtered_YYYY-MM-DD.json`
  - 原始全库（仅作核对，禁止只读前几行）：`https://arthurwang2.github.io/TrendRadar2/report/daily-json/YYYY-MM-DD.json`
...
## 端到端流程（必须严格按序执行）
1. **解析语义筛选结果 + AI 分析（禁止只扫 daily-json 开头）**  
   - **必读** `semantic_filtered_YYYY-MM-DD.json`：`scan` 段记录全库扫描条数（热榜+RSS 分批语义），`matched_items` 为全部匹配新闻（可能几十条，须读完整个数组），`v3.class1/2/3` 为 V3 合格项。  
   - 若存在 `ai_analysis_*.json`：用其 `core_trends` / `signals` / `rss_insights` 写「今日要点」。  
   - daily-json 仅用于对 Top3 条目补 url/summary，不得用「读前 N 条」代替语义筛选。  
